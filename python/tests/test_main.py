...
def test_usage_errors(capsys, tmp_path):  # R-W1, R-X1
    with pytest.raises(SystemExit) as e:
        play_main([])
    assert e.value.code == 2 and "usage: odca <file> [<file> ...] [--shuffle] [--longest] [--play-survivors] [--fullscreen] [--4] [--3] [--2] [--1] [--watchdog N] [--grace N] [--cells N] [-x N] [--experiment N]" in capsys.readouterr().out
    with pytest.raises(SystemExit) as e:
...
def test_odca_flags_are_parsed(monkeypatch, tmp_path):  # R-U2, R-X1
    ...
    assert calls == [({"show": show, "shuffle": False, "longest": False, "play_survivors": False, "experiment": 0, **clocks}, True, 2),  # 2-point cells by default
                     ({"show": show, "shuffle": True, "longest": False, "play_survivors": False, "experiment": 0, **clocks}, False, 2),
                     ({"show": show, "shuffle": False, "longest": False, "play_survivors": False, "experiment": 0, **clocks}, False, 1),
                     ({"show": show, "shuffle": False, "longest": False, "play_survivors": False, "experiment": 0, "play_timeout": 20, "play_grace": 10}, False, 2)]
