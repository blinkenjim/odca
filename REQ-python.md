...
player-side half — `odca` starting a pair from its best recorded seed
(R-X4), and `odca-select` carrying a file's `seeds` section through a
rewrite (R-P3) — is small and due at the next Python release.
