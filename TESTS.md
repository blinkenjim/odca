...
- **M-15** `odca my.odca --longest` after M-14: the window opens 1200
  ...
  in full screen the counter `<n>/<m>` also stands in the lower-left
  corner in yellow with a black outline, ticking with the terminal's,
  and vanishes on leaving full screen while the terminal's goes on
  (R-U11); each seed runs to its
