...
**Files:** `conformance/help-odca.txt`, `conformance/help-odca-select.txt`,
and `conformance/help-odca-evolve.txt` — the normative `--help` texts
(R-U9), printed byte for byte by every implementation that has the
program (Swift has all three; Python has `odca` and `odca-select`, no
`odca-evolve` yet). Editing one is a spec change (bump `REQTS.md`). Each
...
user's real state — see the warning in `REQ-python.md`). An
implementation without section 4e's program (Python, as of 3.63.0) is
exempt from PT-40, PT-41, and PT-46, and from the `odca-evolve` part of
PT-35, as it is from the `lifetimes` vectors; every other row binds it.
...
| PT-25 | R-U3, R-K10 | The history holds `rows` + 1 rows; the scroll offset is 0 while filling, 1 at the default speed once full, the elapsed fraction of the delay (wrapping when a generation is computed) once the delay exceeds twice the initial delay, and 1 while paused; resuming computes exactly one generation on the first tick and returns the offset to 0. The initial delay is a construction parameter (1/60 s at 4-point cells, the session's default parameter, halved per halving of the cell by the launcher, so 1/120 s at the default 2-point cell, R-U5): with 1/240 s the session starts at that delay, two `-` presses reach 1/60 s, and that delay scrolls continuously though it is discrete at the default. |
...
| PT-40 | R-E2, R-A1 | `lifetime`: a block of eight 3s in sixteen cells under a rule that keeps a 3 only between 3s lives 4 generations and ends `state 3 extinct`; capped at 3 it lives 3 and ends `survived`; under the all-zero rule (only state 0 producible) a row is all zeros from generation 1 and ends at generation 2 as `repeating (period 1)`, or survives when capped at 1; a stop asked for at the first look abandons the row (nil), and a row narrower than three cells is refused. A real rule and row of 120 cells goes stagnant at generation 1699, having neither died out nor confirmed a cycle, and ends `stagnant`; capped inside the window it ends `survived` instead; a row that dies out still ends by extinction. The sliding window answers exactly what rescanning the whole window answers, on every prefix of a rising, a falling, a flat and a noisy sequence. A producible state under 10% of the row without a break for `effectiveExtinctionWindow` generations is extinct though cells of it remain, is a living minority holding the verdict back until then, is named ascending beside a state with no cells at all, and has its count reset by any generation that puts it back over the share. |
| PT-41 | R-E3, R-E2, R-E4 | Merging twelve distinct rows keeps the ten longest, longest first; merging a list with itself changes nothing; equal lifetimes order by row text; a row would rank first when longest, sixth between the sixth and seventh, nowhere when equal to the shortest of ten, tenth when nine are kept, and nowhere when already kept; merging seeds by rule and width keeps every list. A search of a fast-dying rule at twelve cells with a 1.5 s budget and two workers, started from one recorded seed: measures many rows, ticks at least once with the time left, keeps at most ten longest first with the recorded seed still on top and no row twice, reports every join with a rank in 1–10, the last of the ten having arrived by a join, and is stopped when done; a search of an immortal rule stopped from its first tick returns within seconds with nothing kept (the rows in flight discarded). Ten survivors held, and only ten, end a turn (R-E3): nine, nine and an extinction, or none do not. The clock of R-E4 and R-O16: 0 s is `00:00:00`, 0.2 s rounds up to `00:00:01`, 59.5 s to `00:01:00`, 4 h 5 min 6 s is `04:05:06`, and a negative span is `00:00:00`. |
...
| PT-46 | R-E5 | The plan for a round trip: with rules a–e whose lifetimes at width 8 are {5000, 3000}, {400, 300}, {9000, 4000}, {4000, 3000}, {4500, 4000}, no limit skips a, c, d, e (four ahead of b, each reporting b's 400); limit 2 skips c and e (shortest 4000 each), limit 3 adds a (3000, before d in file order), a limit above the count changes nothing, and a lone rule with seeds is never skipped. The judgment on one rule: with rules a, b, c and lifetimes at width 8 of {5000, 4000, 3000}, {2000, 1500}, and {100000}: a gives up its turn, reporting shortest 3000 against 2000, b's longest, whatever c holds; c gives up its turn against the same 2000; b, furthest behind, runs; without c's seeds a still gives up its turn and c, with none, runs; a holding {2222, 2221} runs (1998.9 is under 2000) and holding {2223} alone gives up (2000.7); at width 9 a runs (no seeds there); with no other rule holding seeds a runs; a rule outside the file's pairs holding 5000 does not count. |
| PT-47 | R-E1, R-E3 | Several odca files unite into one body of data, losing nothing. Pairs are their content: the same pair under two different names in two files becomes one, under the earlier file's name; two different pairs sharing a name both survive, the earlier file keeping the name and the later renamed past every generated name in either file (files holding pair-0000 and pair-0001, and pair-0000 and pair-0009, give three pairs named pair-0000, pair-0001 and pair-0010, all names distinct). Naming the files the other way round changes only who keeps a contested name, never what survives. Seeds are pooled instead: six seeds at one width in each of two files, none shared and the best in the file named second, yield the ten longest-lived of the twelve, the same list whichever order the files are given in; a width only the later file holds survives untouched. |
| PT-48 | R-P6, R-P3 | A file that is not there yet is not rejected; `{"pairs": []}` and `{}` pass. An empty file and a garbage one are `not valid JSON`; a top-level array names `(the top level is not an object)`; `pairs` or the 3.0.0 `looks` holding a non-list, and `seeds` holding a non-object, are each named. A trailing comma is refused with its line, whether it closes an object across several lines or an array on one, even though JSONSerialization accepts it and Python's json does not. A comma inside a string is not one, including before a `}` and after an escaped quote. |
| PT-49 | R-M12 | The experimental rule classes: table sizes 20, 80, 20, 35; counted cells 3, 3, 3, 4; dense sizes 49, 49, 49, 101; weights 0,1,5,25 for four counted cells and 0,1,4,16 for three. Class 1 with each class-0 entry repeated four times is identical to that class-0 rule over 200 generations. Class 2 is reversible: 300 generations forward, then 300 walked back by `rule[counts(previous)] - current`, returns both rows exactly. Every class round-trips through its rule ID, mutates within its class, keeps its class through a mutation, steps to rows of the width with states under 4, and reports a state of one row for class 0 and two for the rest. |
| PT-50 | R-P3, R-M12 | Every rule class round-trips through an odca file: a pair carries `experiment` when its class is not the ODCA and no key at all when it is, so a file of ordinary pairs holds no such key and the shipped files re-save byte-identically. Two pairs whose 20 digits match under different classes are two pairs, keyed apart in `seeds` as `<class>:<id>`, and keep separate seeds. A pair naming an unknown class is skipped, as is one whose rule ID has the wrong number of digits for the class it names. |
...
- **M-9** At speeds below half the starting rate (60 generations per
  second at the default 2-point cell, 30 at 4 points) the picture slides
  continuously rather than stepping; pausing and resuming produce no
  visible jump (R-U3, R-K10).
- **M-11** `odca --help`, `odca-select --help`, and `odca-evolve --help`
  (Swift; Python has no `odca-evolve`) print their texts and exit at
  once, with no window, no toolkit banner, and no change to `~/.odca`;
  `odca` alone prints a usage line (R-U9).
...
- **M-14** `odca-evolve my.odca --cells 600 --time 60` on a terminal:
  `round trip 1`, each rule's line, the countdown (`00:00:59  1234 seeds/s` downward, in
