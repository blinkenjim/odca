...
| PT-47 | R-E1, R-E3 | Several odca files unite into one body of data. Pairs go by name: two files each holding `shared` under different rules yield the earlier file's, with the order that of first appearance and each file's own names all present; naming the files the other way round hands the clash to the other file. Seeds are pooled instead: six seeds at one width in each of two files, none shared and the best in the file named second, yield the ten longest-lived of the twelve, the same list whichever order the files are given in; a width only the later file holds survives untouched. |
