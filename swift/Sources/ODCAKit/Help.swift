...
keeps a contested name, never what survives.

-o says where the results go and is always required, however many files
are given. It may name one of the input files, which is the ordinary way
to go on searching one file, but that overwrites it and so is warned
about.
...
period is confirmed (as odca's own detector confirms one), or the minority
population has held steady long enough to count as stagnant (as it does on
screen), or the cap. The
...
holds one for exactly the width on screen.
  ...
  --cap N         a row still alive after this many generations counts as
                  having survived and stops there; 1000000 by default. Once
  ...
  --limit N       with --parity: at most N rules give up their turn per
                  ...
                  limit
  -o FILE         where to write the results; required. May name one of
  ...
  -v, --verbose   say more, and only say more: nothing about what is
                  searched or written changes. Above each row that joins
                  the ten, and again as a rule's turn ends, names the
                  pairs the rule belongs to — one line each, the pair's
                  name, its colour set and the rule, the first of them
                  also carrying the width — and under the join the ages
                  of the ten as they stand, the row that has just
                  arrived marked with an asterisk. Each burst opens
