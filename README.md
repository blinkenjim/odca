...
| `platformio/` | early MCU port, two boards: the engine (R-M), boring detector (R-A), and display run together on the RP2350 and on the CYD (ESP32-2432S028), the former by full redraw and the latter by hardware scroll in portrait; see `platformio/README.md` |
...
- **`odca-evolve <file.odca> --cells N --time SECONDS [--cap N] [--parity [--limit N]]`** (Swift
  ...
  `--limit N` caps the skips per round trip at the N furthest ahead.
...
| a   | toggle auto-init: once every row on screen is boring (a producible state extinct with no minority state still alive — a state under 10% of the row for 12800 generations without a break counting as extinct though cells of it remain — or a cycle of any period (detected by Brent's algorithm, period printed) or a row repeating one from the last 4000 generations, or a minority population stagnant for 1600 generations — fixed counts, independent of the window), re-initialize the cells as `i` does; on at startup |
| q   | quit                                         |
