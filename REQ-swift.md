...
| `ODCAKit` (library) | engine `Rule`/`Automaton` (R-M), `Classifier` (R-C), `CandidateSearch` (R-S), `Store` + `ColorSet` + `Pair` (R-P), `Show` + `Segment` (R-X1, R-X7: scripts to a show through `CShow`), `Session` (R-U/K/B/A/W/X/O orchestration), `Xoshiro256` (R-N1), the help texts |
