...
Version 3.78.3 — 2026-09-19 (`odca-evolve` `-v`/`--verbose`: `say(blankBefore:)` opening each burst, the blank printed inside the same lock as its line; `pairsByRule` and `pairLines` in ODCAEvolve/main.swift, and `SeedSearch.onKept` now hands over the ten as a third argument, since it fires with the search's lock held and reading `kept` would deadlock on it; always requires `-o`, warning when it names an input, compared through `resolvingSymlinksInPath`; takes several files: `Store.loadOdcaFiles` unions pairs by content, renaming a later pair whose generated name is already taken via `Store.generatedPairNumber` (which `nextPairName` now shares), `Store.loadSeeds([URL])` folds the files through `Evolve.merge` so seeds are pooled and the best ten kept regardless of order, `parseArguments(many:)` with `-o` and `valueNames` for its usage placeholder, the output URL chosen in `ODCAEvolve/main.swift` before any file is read; `repeatWindow`/`stagnationWindow` are fixed generation counts, no `rows` in `observe`; the audit's spec and help-text fixes; `n`/`p` live while paused in odca; the parity plan line and the running place on each rule line; ten survivors end a turn early, `Evolve.allSurvived` checked at each tick; round trips until interrupted; `Evolve.parityPlan` made per round trip, `--limit`; `--parity` against the other rule furthest behind; Brent's cycle detection in `Evolve.lifetime`, and a confirmed cycle ends a `--longest` item; `odca-evolve --parity`: `Evolve.givesUpTurn`, judged in the rule loop on the in-memory seeds; `odca-select --longest`: `Session(selectLongest:)`, `selectSeeds`, `shown` filtering `rebuildViewOrder`, `deletePair` dropping a rule's seeds; `o` and the counter in any window; the title-bar double-click through a local mouse-down monitor in the app delegate; the counter at 18 points bold, every frame; the on-screen counter: `CounterView`, an NSView over the grid drawing an attributed string with a negative stroke width; the `--longest` grid stretched to the view's width: `Geometry.fitToWidth`, the layer filters; the `--longest` window is set to the seeds' width after launch, since SwiftUI restores the last frame on its own; `Session.counter` on the terminal through `TerminalStatus`, refreshed at `ViewerModel.counterInterval`; 3.46.0: the counter in the title; 3.44.0: `odca --longest`: `Session(longest:)`, `Show.seedWidth`, a fixed-width window through `ViewerModel.fixedCols`; 3.42.1: the countdown rate over the last `Evolve.rateWindow` ticks, from `SeedSearch.tried` samples; 3.42.0: rows tested per second; 3.40.1: the countdown at the left margin; 3.40.0: `odca-evolve` prints the ten ages as a rule ends; 3.38.1: clocks as `hh:mm:ss`, the time left opening every line; 3.36.0: `odca-evolve`, the `ODCAEvolve` target, `Evolve` and `SeedSearch`, seeds in `Store` and on arrival in `Session`; argument parsing moved into ODCAKit; 3.32.0: `shuffle` a statement of its own; 3.30.0: a script shuffles its pairs; 3.28.0: play scripts: `Show`, the `CShow` target; 3.26.0: pairs, named; 2-point default; 3.24.0: navigation scrolls the pair in; 3.22.0: a mutated pair saves as a new pair; 3.20.0: `--3`; 3.18.0: `U` undoes all; 3.16.0: `m` edits the pair under review; 3.14.0: `--watchdog`, `--grace`; 3.12.0: shuffle constraints; 3.10.0: initial delay scales with the cell; 3.8.0: cell size flags; 3.6.0: `F` toggles full screen; `swift/run` wrapper; 3.4.0: `--fullscreen`; 3.2.0: per-row palette table, rows keep their colors for good; 3.0.1: window fix for file arguments; 3.0.0: two executables, `odca` and `odca-select`, over a shared `ODCAUI` module; odca files and `library.json`)
...
- **odca --longest** (R-X8): `Session(show:shuffle:longest:)` builds the
  ...
  default window size uses it, and once the window exists the app
  delegate sets its content width outright (SwiftUI restores the last
  run's frame by itself, autosave name or not, which 3.48.0 missed: an
  ordinary run's wider window came back with bars); `AutomatonView`
  scales the grid to the view's width (`Geometry.fitToWidth` gives the
  factor and the rows for `layout`, and `refresh` sizes the image layer
  in scaled cells with nearest filtering for a whole-number factor and
  linear otherwise), paints the margins black, and skips the resize
  increments.
